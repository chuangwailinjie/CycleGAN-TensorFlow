...
def discriminator(image, options, reuse=False, name="discriminator"):
    ...
    with tf.variable_scope(name):
        ...
        h0 = lrelu(conv2d(image, options.df_dim, name='d_h0_conv'))#下面这些128,64,32都是设定的特征图的深度
...
def generator_unet(image, options, reuse=False, name="generator"):#cycleGAN的生成器使用了和pix2pix相同的UNET以及ResNet block
#这里是生成器，要把输入的图像X经过映射转换到Y域中的图像，如何进行转换，首先是前面的大概8个卷积网络来生成高阶特征
#然后就是deconv反卷积，反卷积来尽力重构原来的图像，这就是UNet
#UNet是一种Encoder-Decoder模型，所以这里命名为e,d
    with tf.variable_scope(name):
        # image is 256 x 256 x input_c_dim
        if reuse:
            tf.get_variable_scope().reuse_variables()
        ...
        #最后一层没有skip-connection，保证输出的shape
...
def generator_resnet(image, options, reuse=False, name="generator"):
    ...
    with tf.variable_scope(name):
        ...
        def residule_block(x, dim, ks=3, s=1, name='res'):
            ...
            #注意这里使用了tf.pad，是因为残差网络需要保持输入和输出的size完全的一致
            return y + x#ResNet残差网络block，中间是两层卷基层，最后一层以一个输入的x直接skip connection到输出层来模拟
        ...
        #上面保持的layer输入输出size都是gf_dim*4，所以使用两个反卷积layer，每次都讲size缩小一辈
        #两次之后，讲layer的输出重置到gf_dim
